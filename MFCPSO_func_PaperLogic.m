function [gbest,gbestval,fitcount,suc,suc_fes]= MFCPSO_func_PaperLogic(jingdu,func_num,fhd,Dimension,Particle_Number,Max_Gen,Max_FES,VRmin,VRmax,varargin)
%%
...
if ~isempty(Fixed_p)
    % 如果你有強制指定 p (例如 0.9)
    % 這裡你可以決定要留多少給 Novelty，例如給 0 (完全不看 Maverick) 或 0.05
    p_ratio_novelty = zeros(1, ps); % 設為 0，代表 p=0.9 時，Maverick 比例歸零
else
    % 沒指定時，跑作者原本的邏輯
   % 修改後的代碼 (加入 max 保護機制)
